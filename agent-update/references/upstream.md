...
- Use a `git-secret`-encrypted, per-host `agent-file-sync.yaml` to choose between the host-global and sibling-checkout (`moreh-metal`) layouts above, rather than a single layout fixed in `AGENTS.md`/`SKILL.md`: the choice is something the user grants explicitly per host, by adding it to the encrypted file and sharing the decryption key with that host, not something an agent-update run on any host can redefine for every host at once. Use the portable per-skill-link layout above rather than upstream-specific `~/.files`, Moreh-server paths, tool-specific clones, or whole-directory Codex or Claude skill links.
