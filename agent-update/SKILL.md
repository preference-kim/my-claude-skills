---
name: agent-update
description: Synchronize the shared dotfiles AGENTS.md and agent skills into the sibling moreh-metal checkout while preserving intentional local policy. Use for daily session refreshes, /agent-update or $agent-update requests, agent-instruction edits, project-link repair, upstream reconciliation, or committing and pushing shared agent-file changes.
---
...
4. Resolve `<dotfiles>/../moreh-metal` as the managed project checkout. If it exists, require `git -C <candidate> rev-parse --show-toplevel` to resolve to that same directory. Do not search for, create, or clone an alternate checkout when it is absent or invalid; report that project synchronization could not run.
5. Read [references/upstream.md](references/upstream.md) before comparing or updating upstream content.

## Choose the mode

- **Daily refresh:** When the canonical AGENTS requests the session-start refresh, skip network and repository work if `${XDG_STATE_HOME:-$HOME/.local/state}/agent-update/last-successful-sync` contains today's local date. Still verify and repair the `moreh-metal` entry points and complete project-skill manifest. Otherwise run the full refresh.
...
- **Link repair:** When asked to install or repair shared agent files, run the `moreh-metal` symlink checks even if today's refresh already succeeded.
...
## Maintain the moreh-metal entry points and skill installations

Do not create or repair host-global entries below `~/.codex` or `~/.claude`. Keep these project-local instruction entry points when the managed `moreh-metal` checkout is available:
...
2. Run `git diff --check` in both repositories. Run the installed skill validator when available and verify the required SKILL.md frontmatter directly otherwise. Re-run the full `moreh-metal` instruction-link and project-skill manifest comparison after link repair.
...
7. Write today's date and the reviewed upstream commit to `last-successful-sync` only after both pushes succeed or when no tracked change was needed, and only after the `moreh-metal` entry points and complete project-skill manifest pass verification.
8. Re-read the final AGENTS.md and SKILL.md for the current session. Report changed files, commit IDs, push results, repaired project links, and any verification limitation. Keep a no-change daily refresh unobtrusive.
