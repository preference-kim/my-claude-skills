...
1. Fetch the recorded reviewed commit and current `master` commit for both upstream paths listed in `references/upstream.md`.
2. Compare each old document with its current version. If neither changed, continue to requested edits or finish the refresh.
3. Classify each changed idea as applicable, covered by an intentional divergence, or a new conflict.
4. Integrate applicable changes into the surrounding local document so it reads as one design. Preserve the rationale of every intentional divergence, not merely its current wording.
5. If a new change contradicts local policy or has ambiguous operational impact, leave tracked files and the reviewed baseline untouched, report the exact conflict, and stop automatic synchronization.
...
3. Review both diffs and stage only agent-update files in the skills repository and the canonical guidance, symlinks, `.gitmodules`, or submodule pointer in dotfiles.
